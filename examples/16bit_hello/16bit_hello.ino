...
#include <SparkFun_PicoDVI.h>         // Core display & graphics library
...
// 'pimoroni_demo_hdmi_cfg') or the SparkFun RedBoard IoT - RP2350 -
...
void loop()
{
    ...
    if (++rotate > 3)
        rotate = 0;              // Cycle through screen rotations 0-3
...
void show_shapes()
{
    ...
    display.drawTriangle(cx + pad, cy - pad, cx + pad + half, cy - minor, cx + minor - 1, cy - pad, 0x07E0);
    ...
    display.fillTriangle(cx + pad, cy - pad, cx + pad + half, cy - minor, cx + minor - 1, cy - pad, 0x07E0);
...
typedef enum
{ // Alignment options passed to functions below
    GFX_ALIGN_LEFT,
    GFX_ALIGN_CENTER,
    GFX_ALIGN_RIGHT
} GFXalign;
...
void print_aligned(Adafruit_GFX &gfx, const __FlashStringHelper *str, GFXalign align = GFX_ALIGN_LEFT)
{
    uint16_t w, h;
    int16_t x, y, cursor_x, cursor_x_save;
    cursor_x = cursor_x_save = gfx.getCursorX();
...
void print_aligned(Adafruit_GFX &gfx, const String &str, GFXalign align = GFX_ALIGN_LEFT)
{
...
void show_char_map()
{
    ...
    if (cell_size < 8)
        return;                            // Screen is too small for table, skip example.
    ...
    if (y >= 4)
    {                                                      // If there's a little extra space above & below, scoot table
    ...
    { // 16 down...
        ...
        { // 16 across...
            if ((row + col) & 1)
            { // Fill alternating cells w/gray
                display.fillRect(x + (col + 1) * cell_size, y + (row + 1) * cell_size, cell_size, cell_size, 0x630C);
            }
            ...
            display.drawChar(x + (col + 1) * cell_size + inset_x, y + (row + 1) * cell_size + inset_y, row * 16 + col,
                             0xFFFF, 0xFFFF, 1);
        }
    }
...
void show_custom_text()
{
    ...
    if (CORNER_RADIUS && (display.height() == 280))
        h += 20;
...
PROGMEM const uint8_t hex_bitmap[(HEX_WIDTH + 7) / 8 * HEX_HEIGHT] = {
    0b00000001, 0b10000000, 0b00000111, 0b11100000, 0b00011111, 0b11111000, 0b01111111, 0b11111110,
    0b01111111, 0b11111110, 0b01111111, 0b11111110, 0b01111111, 0b11111110, 0b01111111, 0b11111110,
    0b01111111, 0b11111110, 0b01111111, 0b11111110, 0b01111111, 0b11111110, 0b01111111, 0b11111110,
    0b01111111, 0b11111110, 0b00011111, 0b11111000, 0b00000111, 0b11100000, 0b00000001, 0b10000000,
};
...
void show_bitmap()
{
    ...
    const uint8_t steps = min((display.height() - HEX_HEIGHT) / Y_SPACING, display.width() / HEX_WIDTH - 1) / 2;
    ...
    for (uint8_t a = 0; a <= steps; a++)
    {
        for (uint8_t b = 1; b <= steps; b++)
        {
            ...
            display.drawBitmap( // UL section centered green: a = blue, b = red
                center_x - b * HEX_WIDTH + a * HEX_WIDTH / 2, center_y - a * Y_SPACING, hex_bitmap, HEX_WIDTH,
                HEX_HEIGHT, display.color565(255 - 255 * b / steps, 255, 255 - 255 * a / steps));
            display.drawBitmap( // LL section centered blue: a = red, b = green
                center_x - a * HEX_WIDTH + b * HEX_WIDTH / 2, center_y + b * Y_SPACING, hex_bitmap, HEX_WIDTH,
                HEX_HEIGHT, display.color565(255 - 255 * a / steps, 255 - 255 * b / steps, 255));
        }
    }
...
void show_canvas()
{
    ...
    { // For each label...
        ...
        if (w > max_w)
            max_w = w; // Keep track of widest label
    }
    ...
    if (canvas16.getBuffer())
    {
        ...
        for (uint8_t i = 0; i < 3; i++)
            print_aligned(display, label[i], GFX_ALIGN_RIGHT);
        ...
        while ((elapsed = (millis() - startTime)) <= PAUSE * 2)
        {
            ...
            {                              // For each label...
                ...
                display.drawRGBBitmap(max_w + PADDING, i * FreeSansBold18pt7b.yAdvance + y_offset, canvas16.getBuffer(),
                                      canvas16.width(), canvas16.height());
            }
        }
    }
    else
    {
        ...
        while ((elapsed = (millis() - startTime)) <= PAUSE * 2)
        {
            for (uint8_t i = 0; i < 3; i++)
            {
                ...
                display.drawBitmap(max_w + PADDING, i * FreeSansBold18pt7b.yAdvance + y_offset, canvas1.getBuffer(),
                                   canvas1.width(), canvas1.height(), color[i], 0);
            }
        }
    }
