...
#define ENCODED_COMMAND(x)                                                                                             \
    ((BIT_MOVE(x, 0, 0)) | (BIT_MOVE(x, 1, 2)) | (BIT_MOVE(x, 2, 4)) | (BIT_MOVE(x, 3, 6)) | (BIT_MOVE(x, 4, 8)) |     \
     (BIT_MOVE(x, 5, 10)) | (BIT_MOVE(x, 6, 12)) | (BIT_MOVE(x, 7, 14)))
...
void setup()
{
    ...
    for (int i = 0; i < 256; i++)
    {
        int j = (BIT_MOVE(i, 0, 0)) | (BIT_MOVE(i, 2, 1)) | (BIT_MOVE(i, 4, 2)) | (BIT_MOVE(i, 6, 3)) |
                (BIT_MOVE(i, 1, 4)) | (BIT_MOVE(i, 3, 5)) | (BIT_MOVE(i, 5, 6)) | (BIT_MOVE(i, 7, 7));
        decode[i] = j;
    }
    ...
    for (;;)
    {
        ...
        if ((ww & 0x2))
        { // DC bit is set, that means it's data (most common case, hence 1st)
            if (bufidx >= 0)
            { // Decode & process only if recv buffer isn't full
