...
const struct
{
    ...
    const uint8_t *mask;
} spritedata[] = {{sprite0, mask0},   {sprite1, mask1},   {sprite2, mask2},   {sprite3, mask3},   {sprite4, mask4},
                  {sprite5, mask5},   {sprite6, mask6},   {sprite7, mask7},   {sprite8, mask8},   {sprite9, mask9},
                  {sprite10, mask10}, {sprite11, mask11}, {sprite12, mask12}, {sprite13, mask13}, {sprite14, mask14},
                  {sprite15, mask15}, {sprite16, mask16}};
...
const float corners[8][3] = {
    // Cube vertices in original coord space
...
{                          // Runs once every frame
    ...
    display.drawGrayscaleBitmap((display.width() - SPRITE_WIDTH) / 2, display.height() - SPRITE_HEIGHT,
                                spritedata[spritenum].sprite, spritedata[spritenum].mask, SPRITE_WIDTH, SPRITE_HEIGHT);
...
{
    return ((x < 0.0) | ((x >= (float)display.width()) << 1) | ((y < 0.0) << 2) |
            ((y >= (float)display.height()) << 3));
}
...
void line(float x1, float y1, float x2, float y2, uint8_t color)
{
    ...
    if (mask1 & mask2)
        return; // line won't overlap screen
    ...
    { // Diagonal
        ...
        { // "Wide" line
            ...
            for (uint16_t x = ix1; x <= ix2; x++)
            {
                ...
                if (iy < display.height())
                    ptr[iy * display.width() + x] = color;
            }
        }
        else
        { // "Tall" line
